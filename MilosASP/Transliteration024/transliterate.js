...
// Defining functions with clicking on the buttons with 'rerune' id and 'select1' id
    ...
    .click(function (event) {
        ...
        $('#source').html(textRight).trigger('autosize.resize');
...
function TransliterateAjax(text, srcLang, dstLang) {				
    ...
    $.ajax({
		    ...
		    success: function (xml) {
				...
				/*if($('#ot-res-data').height() > $('#ot-src-wrap').height()) {
					$('#source').trigger(autosize.ajust($('#ot-res-data').height())');
				}else{
					$('#ot-res-data').height($('#ot-src-wrap').height());
				}*/
		    },
