    ...
    $("ul").menu({
        select: function (event, ui) {            
			...
			if (ui.item.parent().attr("id") == "ui-menu-left") {
				...
				TransliterateAjax($("#result_box").text());
			}
				$('#' + selectedButtonID + '> span').text(ui.item.text().split("-")[1]);
...
$(function () {
    ...
    $("#rerune")
			...
			.next()
				...
				.click(function () {
				    var menu = $(this).parent().next().hide().next().show().position({
				        my: "left top",
				        at: "left bottom",
				        of: this
				    });
    ...
    .click(function (event) {
		...
		$("#ui-menu-left").find(".selected").removeClass("selected").end()
			.find("li:contains('"+langRight+"')").addClass("selected");
		$("#ui-menu-right").find(".selected").removeClass("selected").end()
			.find("li:contains('"+langLeft+"')").addClass("selected");
    });
    ...
    $.ajax(
		{
		    ...
		    success: function (xml) {
				...
				$("#ui-menu-left").find('li:contains("EN-English")').addClass("selected");
				$("#ui-menu-right").find('li:contains("SR-Serbian")').addClass("selected");					        
		    },
